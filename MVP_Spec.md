...
8. **Editor**: JSON block-based output stored as JSONB; no HTML WYSIWYG. **Library DECIDED 2026-07-20: TipTap v3, MIT-licensed surface only, headless, Vue 3 bindings; ProseMirror-direct as named fallback (see ADR-001).** The earlier "commercial licensing costs" concern is resolved: TipTap core is MIT and free — only cloud/platform features (collaboration, document history) are paid, and none are used (versioning is Supabase-side per LD 10; collab deferred). No `@tiptap-pro/*` package ever enters the repo. Blocks and later CCMS variables are our own custom node types (headless = we own schema, nodes, and all UI). Build-binding conditions carried in ADR-001: schema-version stamping + additive-only editor schema + lazy read-time migration (Commitment 3, no silent content loss); per-node Markdown/HTML serialization for export (Commitment 4); role="textbox" + keyboard-operable chrome + ARIA (Commitment 1); Android+IME included in the typing-lag "feel" gate (Commitment 9); read views render from JSON via a static/server serializer, never by mounting the editor, no raw-HTML node, href-protocol whitelist in the Edge Function validator; optimistic base-version concurrency check on saves; exact version pins with committed lockfile, upgrades batched as schema-migration events. There is no autosave; the deliberate Save action is the only persistence path and publishes the immutable version row (LD 10), with a cheap unsaved-changes guard so work isn't lost on navigation.
...
- ~~**Editor library**~~ **DECIDED 2026-07-20: TipTap v3, MIT-surface-only; LD 8 amended, see ADR-001.**
